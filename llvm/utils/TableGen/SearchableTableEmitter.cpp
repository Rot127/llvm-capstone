...
bool SearchableTableEmitter::compareBy(Record *LHS, Record *RHS,
  ...
  for (const auto &Field : Index.Fields) {
    ...
    if (isa<BitsRecTy>(Field.RecType) || isa<IntRecTy>(Field.RecType)) {
      ...
      std::string LHSs = PI.searchableTablesPrimaryRepresentation(Index.Loc, Field, LHSI, getIntrinsic(LHSI));
      std::string RHSs = PI.searchableTablesPrimaryRepresentation(Index.Loc, Field, RHSI, getIntrinsic(RHSI));
...
void SearchableTableEmitter::emitLookupFunction(const GenericTable &Table,
                                                const SearchIndex &Index,
                                                bool IsPrimary) {
  PI.searchableTablesEmitLookupDeclaration(Table, Index);
  PI.emitString(" {\n");
    ...
    for (const auto &Entry : Entries) {
      ...
      for (const auto &Field : Index.Fields) {
        std::string Repr = PI.searchableTablesPrimaryRepresentation(
            ...
            getIntrinsic(Entry.first->getValueInit(Field.Name)));
        if (isa<StringRecTy>(Field.RecType))
...
void SearchableTableEmitter::emitGenericTable(const GenericTable &Table) {
  ...
  if (Table.PrimaryKey) {
    PI.searchableTablesEmitLookupDeclaration(Table, *Table.PrimaryKey);
  }
  for (const auto &Index : Table.Indices) {
    PI.searchableTablesEmitLookupDeclaration(Table, *Index);
  PI.searchableTablesEmitEndif();
  }
  ...
  for (unsigned i = 0; i < Table.Entries.size(); ++i) {
    ...
    for (const auto &Field : Table.Fields)
      PI.searchableTablesEmitMapIII(Table, LS, Field,
        getIntrinsic(Entry->getValueInit(Field.Name)), Entry);
...
void SearchableTableEmitter::run() {
  ...
  for (const auto &Enum : Enums) {
    ...
    PI.searchableTablesEmitIfdef(Guard);
