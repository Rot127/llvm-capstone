...
void InstrInfoEmitter::run() {
  ...
  for (const CodeGenInstruction *II : Target.getInstructionsByEnumValue()) {
    ...
    if (!Uses.empty()) {
      ...
      if (!IL) PI.instrInfoPrintDefList(Uses, IL = ++ListNumber, getQualifiedName);
    }
    std::vector<Record*> Defs = Inst->getValueAsListOfDefs("Defs");
    if (!Defs.empty()) {
      unsigned &IL = EmittedLists[Defs];
      if (!IL) PI.instrInfoPrintDefList(Defs, IL = ++ListNumber, getQualifiedName);
    }
  }
...
void InstrInfoEmitter::emitRecord(const CodeGenInstruction &Inst, unsigned Num,
  ...
  PI.instrInfoEmitUseDefsLists(EmittedLists, UseList, DefList);
